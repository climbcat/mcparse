  ...
  if (!*flag_defined_common) {
    ...
    *flag_defined_common = 1;
    ...
    if (!strcmp(section, "TRACE")) {
      ...
      cout("  if(isnan(p)  ||  isinf(p)) ABSORB;");
      cout("  if(isnan(t)  ||  isinf(t)) ABSORB;");
      cout("  if(isnan(vx) || isinf(vx)) ABSORB;");
      cout("  if(isnan(vy) || isinf(vy)) ABSORB;");
      cout("  if(isnan(vy) || isinf(vy)) ABSORB;");
      cout("  if(isnan(x)  ||  isinf(x)) ABSORB;");
      cout("  if(isnan(y)  ||  isinf(y)) ABSORB;");
      cout("  if(isnan(z)  ||  isinf(z)) ABSORB;");
      cout("#else");
      ...
      cout("  if(isnan(vy) || isinf(vy)) printf(\"NAN or INF found in vz, %s (particle %lld)\\n\",_comp->_name,_particle->_uid);");
...
cogen_header(struct instr_def *instr, char *output_name)
{
  ...
  cout("  long flag_nocoordschange;   /* set to true if neutron is jumping */");
